 ...
 */
 const intervalIntersection = (firstList, secondList) => {
    ...
    while(firstPointer < firstListLength && secondPointer < secondListLength){
        let intervalA = firstList[firstPointer]
         let intervalB = secondList[secondPointer]
         
         if(intervalB[0] <= intervalA[1] && intervalB[1] >= intervalA[0]){
             tempIntersection.push( [Math.max(intervalB[0], intervalA[0]),Math.min(intervalB[1], intervalA[1])])
